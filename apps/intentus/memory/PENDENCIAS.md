...
- **P-196:** ~~Postgres crash em distribuidoras grandes~~ — **RESOLVIDO 2026-04-28** (commit 52269dc). Workflow run 66454684277 derrubou Supabase no Cemig-D (CSV MT 761MB + BT 777MB) com `EDBHANDLEREXITED` — 100 distribuidoras seguintes falharam em cascata. Fix: `_chunked_insert` divide INSERT em batches de 50k rows com `SET LOCAL statement_timeout = '900s'` em transação curta. Aplicado em `load_mt_csv` e `load_bt_csv`. Reset de Celesc-Dis + CELPA aplicado em produção. Próximo run nacional vai processar Cemig-D + Enel SP (1.8GB) + Coelba (2.5GB) sem derrubar Postgres.
- **P-197:** Camada **SiCAR WFS** (Cadastro Ambiental Rural) — endpoint público `https://geoserver.car.gov.br/geoserver/sicar/wfs`. Sistema oficial federal de propriedades rurais com **APP**, **Reserva Legal** e hidrografia oficial federal. Avaliar layers do GetCapabilities + integrar como `sicar_app`, `sicar_rl`, `sicar_hidro` no painel Mapa & Camadas (substituindo Overpass APP/hidrografia que é OSM-derivado). Mais preciso pra compliance Lei 12.651/2012 (Código Florestal). Sprint dedicado, depois P-191/P-193 estabilizarem.
- **P-198:** Reinserção do **CAD Studio / Projeto Executivo** — botão removido do header em commit ad09091 porque levava à rota quebrada `/parcelamento/:id/cad`. Decisão (2026-04-28): reinserir como **aba dedicada `Projeto Executivo`** entre "Análise 3D" e "Financeiro" (não botão), porque vai precisar de viewport completo (canvas + sidebar + layers + memorial + assinatura). Plano em 4 fases:
  - **Fase 1 (1 sessão):** esqueleto — adicionar `{ key: "projeto-executivo", label: "Projeto Executivo" }` em TABS + componente `<TabProjetoExecutivo>` placeholder com empty state ("Carregue Tier 2 HD") + CTA disparar HD; deprecar rota `/cad` → redirect pra `?tab=projeto-executivo`
  - **Fase 2 (2-3 sessões):** Upload DXF/DWG + canvas Three.js/OpenLayers com overlays automáticos (BDGD HD `bdgd_segments_hd`, topografia, LT/DUP, futuro SiCAR)
  - **Fase 3 (2 sessões):** Memorial descritivo PDF automático (coordenadas UTM dos vértices, distância de servidão, quadro de áreas, confrontações) + assinatura BRy Easy Signer
  - **Fase 4 (longo prazo):** Export DXF padrões municipais + workflow ART/RRT + submissão API prefeitura
  - **Pontos de entrada secundários** (CTAs contextuais): link no painel BDGD quando HD loaded, link no painel Topografia após cut/fill, badge no Visão Geral score, menu kebab `⋮` no header
  - **Bloqueadores:** P-130 (provedor LLM, geral) + P-188 (GITHUB_TRIGGER_TOKEN pra disparar HD)
