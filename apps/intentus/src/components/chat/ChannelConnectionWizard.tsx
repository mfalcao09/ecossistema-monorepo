...
import { Loader2, CheckCircle2, QrCode, RefreshCw, Smartphone, MessageCircle, Instagram } from "lucide-react";
...
export function ChannelConnectionWizard({ open, onOpenChange, mode }: Props) {
  ...
  if (mode === "telegram") {
    return <ManualWizard open={open} onOpenChange={onOpenChange} channelType="telegram" label="Token do Bot" placeholder="123456:ABC-DEF1234..." />;
  }
  if (mode === "webchat") {
    return <ManualWizard open={open} onOpenChange={onOpenChange} channelType="webchat" label="Nome do Canal" placeholder="Chat do Site" />;
  }
  return null;
}

// ==================== QR Code Wizard (Z-API transparent) ====================
function QRCodeWizard({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const callEdge = useCallback(async (action: string, payload: Record<string, unknown>) => {
    const { data: { session } } = await supabase.auth.getSession();
    const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/channel-connect`, {
      method: "POST",
      headers: {
  ...
  const handleRefreshQR = async () => {
    ...
    setLoading(true);
    try {
      const { qrcode: qr } = await callEdge("zapi_get_qrcode", { channel_id: channelId });
      setQrcode(qr);
    } catch {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const data = await callEdge("zapi_check_status", { channel_id: channelId });
        if (data.connected) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "name" && "Dê um nome para o canal e escaneie o QR Code com seu celular"}
            {step === "qrcode" && "Abra o WhatsApp no celular, vá em Aparelhos Conectados e escaneie o código"}
        ...
        {step === "name" && (
          <div className="space-y-4">
            ...
            <Button onClick={handleCreateAndGetQR} disabled={loading} className="w-full">
...
function HunionWizard({ open, onOpenChange, mode }: { open: boolean; onOpenChange: (v: boolean) => void; mode: ChannelConnectionMode }) {
  ...
  const handleFacebookLogin = async () => {
    ...
    try {
      ...
      if (!app_id) {
        toast.error("Meta App ID não configurado. Configure o secret META_APP_ID.");
        setLoading(false);
        return;
      }
      ...
      window.FB?.login(
        async (response: any) => {
          if (response.authResponse?.code) {
            try {
              const result = await callEdge("hunion_exchange_token", { code: response.authResponse.code });
              setChannelId(result.channel_id);

              // List assets
              const assetsData = await callEdge("hunion_list_assets", { channel_id: result.channel_id });
        ...
        },
        {
          ...
          extras: {
            setup: {},
            featureType: mode === "hunion_instagram" ? "page_messaging" : "whatsapp_embedded_signup",
  ...
  const handleRegister = async () => {
    ...
    try {
      await callEdge("hunion_register_channel", {
        ...
        name: selectedAsset.name || selectedAsset.verified_name || "Canal Oficial",
        ...
        channel_type: mode === "hunion_instagram" ? "instagram" : "whatsapp_oficial",
      });
      setStep("done");
      qc.invalidateQueries({ queryKey: ["chat_channels"] });
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {step === "login" && (isWhatsApp ? "WhatsApp Business (Oficial)" : "Instagram / Messenger")}
          ...
          {["login", "select", "done"].map((s, i) => (
            <div key={s} className="flex items-center gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium ${
                ...
                ["login", "select", "done"].indexOf(step) > i ? "bg-emerald-500 text-white" :
        ...
        {step === "login" && (
          <div className="flex flex-col items-center gap-4 py-4">
            ...
            <Button onClick={handleFacebookLogin} disabled={loading} className="bg-[#1877F2] hover:bg-[#166FE5] text-white">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
                ...
                waba.phone_numbers?.data?.map((phone: any) => (
                  ...
                  >
                    <CardContent className="p-3 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">{phone.verified_name || waba.name}</p>
                ...
                <CardContent className="p-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {page.instagram_business_account?.data?.username && (
                      <p className="text-xs text-muted-foreground">@{page.instagram_business_account.data.username}</p>
                    )}
                  </div>
                </CardContent>
              </Card>
            ...
            {(!assets.businesses?.length && !assets.pages?.length) && (
              <p className="text-center text-muted-foreground text-sm py-4">
                Nenhuma conta encontrada. Verifique as permissões da sua conta Meta.
              </p>
            )}

            <Button onClick={handleRegister} disabled={!selectedAsset || loading} className="w-full">
              {loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
...
function ManualWizard({ open, onOpenChange, channelType, label, placeholder }: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>{channelType === "telegram" ? "Telegram" : "Webchat"}</DialogTitle>
          <DialogDescription>Configure os dados do canal</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <Label>Nome do Canal *</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Bot Vendas" />
          </div>
          <div>
            <Label>{label}</Label>
