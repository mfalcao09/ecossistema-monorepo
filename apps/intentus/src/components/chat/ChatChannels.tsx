...
import { Plus, Wifi, WifiOff, Trash2, MessageCircle, Instagram, Send as TelegramIcon, Globe, QrCode, RefreshCw } from "lucide-react";
import { useChatChannels, useUpdateChatChannel } from "@/hooks/useChat";
import { toast } from "sonner";
import { ChannelTypeChoiceDialog, type ChannelConnectionMode } from "./ChannelTypeChoiceDialog";
...
const PROVIDER_BADGES: Record<string, { label: string; className: string }> = {
  hunion: { label: "API Oficial", className: "bg-blue-500/10 text-blue-600 border-blue-200" },
  zapi: { label: "WhatsApp Web", className: "bg-emerald-500/10 text-emerald-600 border-emerald-200" },
  manual: { label: "Manual", className: "" },
};

export function ChatChannels() {
  const [choiceOpen, setChoiceOpen] = useState(false);
  const [wizardMode, setWizardMode] = useState<ChannelConnectionMode | null>(null);
      ...
      {(channels ?? []).length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">Adicione um canal de atendimento para começar</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={channel.id}>
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                  <div className="flex items-center gap-2">
                    <Icon className={`h-5 w-5 ${channel.channel_type.includes("whatsapp") ? "text-emerald-500" : "text-muted-foreground"}`} />
                ...
                <CardContent>
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-sm text-muted-foreground">{CHANNEL_LABELS[channel.channel_type]}</p>
                    {providerBadge && connectedVia !== "manual" && (
                      <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${providerBadge.className}`}>
                        {providerBadge.label}
                      </Badge>
                    )}
                  </div>
                  {channel.phone_number && <p className="text-sm font-mono">{channel.phone_number}</p>}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        updateChannel.mutate(
                          { id: channel.id, status: isConnected ? "desconectado" : "conectado" } as any,
                          { onSuccess: () => toast.success(isConnected ? "Canal desconectado" : "Canal conectado") }
                        );
                      }}
                    >
