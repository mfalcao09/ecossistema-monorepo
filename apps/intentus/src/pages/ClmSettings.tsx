/**
 * CLM Settings — Página de configurações do módulo CLM
 *
 * Rota: /contratos/configuracoes
 * Sidebar: Gestão de Contratos → Configurações CLM (admin/gerente)
 *
 * 9 seções de configuração organizadas em tabs verticais:
 * 1. Campos — Ocultar/adicionar campos do formulário de contratos
 * 2. Aprovação — Cadeia de aprovação padrão + roles customizados
 * 3. Renovação — Prazo, reajuste, checklist de renovação
 * 4. Cláusulas — Categorias, renegociação, comentários inline
 * 5. Documentos — Tipos, tamanho máximo, versionamento
 * 6. Obrigações — Tipos, alertas, recorrência, responsáveis
 * 7. Auditoria — Ações rastreadas, retenção de logs
 * 8. Regras de Aprovação — ApprovalRulesManager (existente)
 * 9. Templates — TemplatesManager (existente)
 *
 * Sessão 50 — Migração do CLMSettingsDialog para página dedicada
 */

import { useState, useEffect } from "react";
import {
  Settings, ArrowLeft, ShieldAlert, Plus, RotateCcw, Save, X, Trash2,
...
export default function ClmSettings() {
  ...
  if (!canManageSettings) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <p className="text-muted-foreground text-sm">Você não tem permissão para acessar as configurações do CLM.</p>
        <Button variant="outline" onClick={() => navigate("/contratos/command-center")}>
          <ArrowLeft className="h-4 w-4 mr-2" /> Voltar ao Command Center
        </Button>
      </div>
    );
  }
  ...
  const tabs = [
    ...
    { value: "regras", label: "Regras Aprovação", icon: <ListChecks className="h-4 w-4" /> },
    { value: "templates", label: "Templates", icon: <FileCode className="h-4 w-4" /> },
  ];
    ...
    <div className="space-y-6 p-4 md:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Central de configuração do ciclo de vida contratual — isolada por empresa
            </p>
          </div>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          <Tabs defaultValue="campos" orientation="vertical">
            <div className="grid grid-cols-[200px_1fr] min-h-[500px]">
              ...
              <div className="p-6 overflow-y-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
                ...
                {/* ── 8. Regras de Aprovação (componente existente) ── */}
                <TabsContent value="regras" className="mt-0">
                  <ApprovalRulesManager />
                </TabsContent>

                {/* ── 9. Templates (componente existente) ── */}
