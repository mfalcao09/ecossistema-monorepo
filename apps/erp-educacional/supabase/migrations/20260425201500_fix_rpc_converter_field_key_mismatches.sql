-- Fix: 4 mismatches de chaves entre o JSONB salvo pelo FormularioRevisao
-- e as chaves que a RPC busca. Detectados no smoke F0.6 (Kauana TESTE) em 2026-04-25.
--
-- FormularioRevisao salva           RPC original busca       Resultado
-- ──────────────────────────────────────────────────────────────────────
-- diplomado.rg                      v_aluno->>'rg_numero'    rg_numero=null
-- diplomado.naturalidade_codigo_    v_aluno->>'codigo_       codigo_municipio_
--   municipio                         municipio_ibge'         ibge=null
-- disciplinas[].docente             v_disc->>'nome_docente'  docente_nome=null
-- disciplinas[].forma_              v_disc->>'forma_         forma_integralizacao
--   integralizacao                    integralizada'          =null
--
-- Solução: aceitar AMBAS as chaves (preferir nova, fallback na antiga
-- pra retrocompatibilidade com extrações IA legadas).
...
BEGIN
  ...
  IF v_diplomado_id IS NOT NULL THEN
    -- ─── UPDATE branch (FIX: aceitar 'rg' e 'naturalidade_codigo_municipio') ───
  ...
  ELSE
    -- ─── INSERT branch (FIX: aceitar 'rg' e 'naturalidade_codigo_municipio') ───
  ...
  -- ─── Disciplinas (FIX: aceitar 'docente' e 'forma_integralizacao') ───
  ...
  -- Step 12: migrar processo_arquivos
  ...
  -- Step 12.5: inserir comprobatórios do acervo
  ...
  -- Step 14: marcar sessão convertida
