/**
 * Sprint 6 — Acervo Digital (sessão 094)
 *
 * Estende a RPC converter_sessao_em_processo para inserir automaticamente
 * em diploma_documentos_comprobatorios os arquivos marcados como
 * destino_acervo = true no momento da criação do processo.
 *
 * Mudanças:
 * - Adiciona step 12.5 (entre migrar arquivos e registrar override)
 * - Retorno JSONB agora inclui `comprobatorios_inseridos`
 *
 * Notas de arquitetura:
 * - diploma_id = NULL — o FK aponta para documentos_digitais (tabela diferente
 *   de diplomas), portanto não usamos. A ligação ao diploma é pelo processo_id.
 * - tipo_xsd: CASE WHEN para mapear valores válidos do enum TTipoDocumentacao
 *   (XSD v1.05). Qualquer valor desconhecido → 'Outros'.
 * - ON CONFLICT (processo_id, arquivo_origem_id) DO NOTHING — idempotência.
 */

-- ════════════════════════════════════════════════════════════════════
-- RPC v3: adiciona step 12.5 — comprobatórios do acervo
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ══════════════════════════════════════════════════════════════════
  -- FIX #1 (v2): Normalizar UFs (nome completo → sigla 2 chars)
  -- ══════════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════════
  -- 11) Inserir ENADE (lê "situacao" OU deriva de "habilitado"/"condicao")
  -- ══════════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════════
  -- 12.5) SPRINT 6 — Inserir comprobatórios do acervo
  --
  -- Para cada processo_arquivo com destino_acervo = true, criar um
  -- registro em diploma_documentos_comprobatorios para que o sistema
  -- possa converter para PDF/A e incluir no XML.
  --
  -- Mapeamento tipo_xsd:
  --   O campo tipo_xsd em processo_arquivos é TEXT (valor livre).
  --   Mapeamos para o enum TTipoDocumentacao do XSD v1.05.
  --   Qualquer valor desconhecido → 'Outros'.
  --
  -- diploma_id = NULL:
  --   A FK aponta para documentos_digitais (≠ diplomas).
  --   A ligação ao diploma é feita via processo_id.
  -- ══════════════════════════════════════════════════════════════════
  ...
  SELECT
    v_processo_id,
    pa.id,
    NULL,  -- diploma_id: ver nota acima
    CASE pa.tipo_xsd
...
COMMENT ON FUNCTION public.converter_sessao_em_processo IS
  'Converte extracao_sessao em processos_emissao+diploma+diplomado+comprobatorios. v3: Sprint 6 acervo. Sessão 094.';
