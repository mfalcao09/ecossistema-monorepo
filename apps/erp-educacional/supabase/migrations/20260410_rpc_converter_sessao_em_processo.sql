/**
 * RPC converter_sessao_em_processo
 *
 * Converte uma extracao_sessao em processos_emissao + diploma + diplomado
 * de forma TRANSACIONAL (tudo ou nada — Postgres cuida do rollback).
 *
 * Chamada por: POST /api/extracao/sessoes/[id]/converter/route.ts
 *
 * Sessão 046 (10/04/2026)
 */
...
BEGIN
  ...
  IF v_diplomado_id IS NOT NULL THEN
    ...
    UPDATE diplomados SET
      ...
      naturalidade_uf       = COALESCE(
        NULLIF(v_aluno->>'naturalidade_uf', ''),
        NULLIF(split_part(COALESCE(v_aluno->>'naturalidade', ''), ' - ', 2), ''),
        naturalidade_uf
      ),
      rg_numero             = COALESCE(NULLIF(v_aluno->>'rg_numero', ''), NULLIF(v_aluno->>'rg', ''), rg_numero),
      rg_orgao_expedidor    = COALESCE(NULLIF(v_aluno->>'rg_orgao_expedidor', ''), rg_orgao_expedidor),
      rg_uf                 = COALESCE(NULLIF(v_aluno->>'rg_uf', ''), rg_uf),
  ...
  ELSE
    ...
    INSERT INTO diplomados (
      ...
      COALESCE(NULLIF(v_aluno->>'rg_numero', ''), NULLIF(v_aluno->>'rg', '')),
      NULLIF(v_aluno->>'rg_orgao_expedidor', ''),
      NULLIF(v_aluno->>'rg_uf', ''),
      NULLIF(v_aluno->>'ra', ''),
  ...
  -- ══════════════════════════════════════════════════════════════════
  -- 11) Inserir dados ENADE (se houver)
  -- ══════════════════════════════════════════════════════════════════
  IF NULLIF(v_enade->>'situacao', '') IS NOT NULL THEN
...
-- ════════════════════════════════════════════════════════════════════
-- Permissão: apenas usuários autenticados via service_role / RPC
-- ════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.converter_sessao_em_processo IS
  'Converte extracao_sessao em processos_emissao+diploma+diplomado de forma transacional. Sessão 046.';
