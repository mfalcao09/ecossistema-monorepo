-- Fix: ON CONFLICT da RPC converter_sessao_em_processo precisa do WHERE
-- matching o índice partial uq_ddc_processo_arquivo
-- (criado em sprint6_acervo_rpc_comprobatorios @ 2026-04-15).
--
-- Sintoma: "there is no unique or exclusion constraint matching the
-- ON CONFLICT specification" ao criar processo novo na UI.
-- Bug oculto desde 15/04 porque ninguém criou processo novo desde então.
-- Detectado no smoke F0.6 (Kauana TESTE) em 2026-04-25.
--
-- Diff cirúrgico (1 linha — Step 12.5 da RPC):
...
BEGIN
  ...
  -- Step 12.5: inserir comprobatórios do acervo (Sprint 6)
  -- FIX 2026-04-25: ON CONFLICT precisa de WHERE matching o índice partial
  -- uq_ddc_processo_arquivo (... WHERE deleted_at IS NULL).
