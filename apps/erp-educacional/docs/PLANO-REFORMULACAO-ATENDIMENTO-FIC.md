...
|---|---|---|
| Meta Cloud API (WhatsApp) | ✅ S2 | `WHATSAPP_TOKEN` + `WHATSAPP_PHONE_NUMBER_ID=<phone-number-id>` + `WHATSAPP_APP_SECRET` + `WHATSAPP_VERIFY_TOKEN=<verify-token>` · WABA ID: `<waba-id>` (valores reais no vault ECOSYSTEM) |
