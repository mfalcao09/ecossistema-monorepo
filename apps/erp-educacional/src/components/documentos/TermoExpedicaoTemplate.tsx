        ...
        >
          ...
          >
            <p className="mb-4">
              ...
              <strong>{dados.ies_municipio}/{dados.ies_uf}</strong>, no exercício
              das atribuições conferidas pela legislação de ensino superior em vigor,
              confere a:
            </p>
            ...
            <div className="my-6 px-4 py-4 border-l-4 border-gray-300">
              <p className="mb-2">
                <span className="text-gray-500" style={{ fontSize: `${tamanhoFonte - 1}pt` }}>Nome:</span>{' '}
                <strong>{dados.nome_diplomado}</strong>
              </p>
              <p className="mb-2">
                <span className="text-gray-500" style={{ fontSize: `${tamanhoFonte - 1}pt` }}>CPF:</span>{' '}
                <strong>{formatarCPF(dados.cpf_diplomado)}</strong>
              </p>
              <p>
                <span className="text-gray-500" style={{ fontSize: `${tamanhoFonte - 1}pt` }}>Curso:</span>{' '}
            ...
            <p className="mb-4">
              ...
              <strong>{dados.titulo_conferido || dados.grau || 'Graduação'}</strong> em{' '}
              <strong>{dados.curso_nome}</strong>, tendo concluído o curso em{' '}
              <strong>{formatarData(dados.data_conclusao)}</strong> e colado grau em{' '}
              <strong>{formatarData(dados.data_colacao)}</strong>, expedido na
              presente data, em <strong>{dataPorExtenso(dataExpedicao)}</strong>.
            </p>

            {/* Registro */}
            {(dados.numero_registro || dados.livro || dados.folha || dados.processo) && (
              <>
                <p className="mb-2 mt-6">
                  <strong>Dados do Registro:</strong>
                </p>
                <table className="w-full mb-4" style={{ fontSize: `${tamanhoFonte - 1}pt` }}>
                  <tbody>
                    {dados.numero_registro && (
                      <tr>
                        <td className="py-1" style={{ width: '30%' }}>Registro nº:</td>
                        <td className="py-1"><strong>{dados.numero_registro}</strong></td>
                      </tr>
                    )}
                    {dados.livro && (
                      <tr>
                        <td className="py-1">Livro nº:</td>
                        <td className="py-1"><strong>{dados.livro}</strong></td>
                      </tr>
                    )}
                    {dados.folha && (
                      <tr>
                        <td className="py-1">Folha nº:</td>
                        <td className="py-1"><strong>{dados.folha}</strong></td>
                      </tr>
                    )}
                    {dados.processo && (
                      <tr>
                        <td className="py-1">Processo nº:</td>
                        <td className="py-1"><strong>{dados.processo}</strong></td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <p className="mb-4">
              Este termo, assinado digitalmente pelos responsáveis da Instituição de
              Ensino Superior, atesta a regularidade do procedimento de emissão do
              diploma digital, nos termos da Portaria MEC nº 70/2025 e da Instrução
              Normativa SESu/MEC nº 05/2022.
            </p>
          </div>
          ...
          <div className="text-center mb-6" style={{ fontSize: `${tamanhoFonte}pt` }}>
            <p>
              {dados.ies_municipio}/{dados.ies_uf}, {dataPorExtenso(dataExpedicao)}
            </p>
          </div>
          ...
          >
            <p className="font-bold mb-3 text-gray-700">Assinaturas Digitais ICP-Brasil:</p>
