...
import {
  ...
  ExternalLink, Shield, ShieldAlert, Users,
} from 'lucide-react'
import Link from 'next/link'
import type { RespostaAuditoria, GrupoAuditoria, IssueAuditoria, AcaoCorrecao } from '@/lib/auditoria/tipos'
...
function BotaoCorrecao({
  ...
  const base = 'inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded border transition-colors'
  ...
  // Ações que levam ao formulário de revisão da extração (novo fluxo)
  // ou ao formulário legado se não houver sessão de extração
  if (
    ...
    // Novo fluxo: sessão de extração disponível → abre formulário de revisão
    if (sessaoId) {
      return (
        <Link
    ...
    // Fallback legado (processos criados manualmente sem extração)
    if (!processoId) return null
    return (
      <Link
        href={`/diploma/processos/${processoId}`}
        target="_blank"
        className={primary}
      >
        <ExternalLink size={9} /> {label}
...
function IssueRow({
  ...
  const severityIcon = {
    critico: <XCircle size={11} className="flex-shrink-0 mt-0.5 text-red-500" />,
    aviso: <AlertTriangle size={11} className="flex-shrink-0 mt-0.5 text-amber-500" />,
    info: <Info size={11} className="flex-shrink-0 mt-0.5 text-blue-500" />,
  }

  return (
    <div className={`flex items-start gap-2 px-3 py-2 rounded-lg border text-[11px] ${severityColors[issue.severidade]}`}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-500">
            {grupo.issues.length} {grupo.issues.length === 1 ? 'problema' : 'problemas'}
          </span>
        </div>
    ...
    <div className={`p-3 rounded-xl border-2 transition-all ${
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-800">Auditar Requisitos XSD</p>
      ...
      {auditoria && (
        <div className="mt-3 space-y-2">
            ...
            <div className="flex items-center gap-3 text-[10px] text-gray-500">
              {auditoria.totais.criticos > 0 && (
                <span className="text-red-600 font-semibold">
                  {auditoria.totais.criticos} crítico{auditoria.totais.criticos !== 1 ? 's' : ''}
                </span>
              )}
              {auditoria.totais.avisos > 0 && (
                <span className="text-amber-600">
                  {auditoria.totais.avisos} aviso{auditoria.totais.avisos !== 1 ? 's' : ''}
                </span>
              )}
            </div>
          )}
          ...
          {auditoria.totais.total > 0 && (
            ...
            >
              {detalhesAbertos ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
