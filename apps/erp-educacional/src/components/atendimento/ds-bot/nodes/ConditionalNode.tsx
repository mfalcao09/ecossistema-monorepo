...
export default function ConditionalNode({
  ...
  const d = data as {
    logic?: "AND" | "OR";
    clauses?: Array<{ left: string; op: string; right?: string | number }>;
  };
  const clauses = d.clauses ?? [];
  return (
    <div
      className={`rounded-lg border-2 bg-white shadow-sm min-w-[220px] max-w-[280px] ${selected ? "border-purple-500" : "border-gray-200"}`}
    >
      ...
      <div className="px-3 py-2 border-b border-gray-100 flex items-center gap-2 bg-gradient-to-r from-purple-50 to-transparent">
        ...
        <span className="text-xs font-semibold text-gray-700">
          Condição ({d.logic ?? "AND"})
        </span>
      </div>
      <div className="px-3 py-2 text-xs text-gray-700 space-y-1">
        {clauses.length === 0 ? (
          <em className="text-gray-400">sem condições</em>
        ) : null}
        {clauses.map((c, i) => (
          <div key={i} className="font-mono text-[11px]">
            <code>{c.left}</code>{" "}
            <code className="text-purple-600">{c.op}</code>{" "}
