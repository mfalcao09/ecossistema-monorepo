...
export default function InputNode({
  ...
  return (
    <div
      className={`rounded-lg border-2 bg-white shadow-sm min-w-[220px] max-w-[300px] ${selected ? "border-emerald-500" : "border-gray-200"}`}
    >
      ...
      <div className="px-3 py-2 border-b border-gray-100 flex items-center gap-2 bg-gradient-to-r from-emerald-50 to-transparent">
        ...
        <span className="text-xs font-semibold text-gray-700">
          Pergunta — {LABELS[t] ?? "Texto"}
        </span>
      </div>
      <div className="px-3 py-2 text-sm text-gray-800">
        <div className="break-words">
          {d.question || <em className="text-gray-400">sem pergunta</em>}
        </div>
        {d.variable && (
      ...
      {t === "input_button" && d.options && d.options.length > 0 ? (
        <div className="px-3 py-2 border-t border-gray-100 space-y-1 relative">
          {d.options.map((opt) => (
            <div
              key={opt.id}
              className="relative text-xs bg-emerald-50 border border-emerald-100 rounded px-2 py-1"
            >
              {opt.label}
              <Handle
                type="source"
