...
import type { DsBotFlow, DsBotNode, DsBotEdge, DsBotNodeType } from "@/lib/atendimento/ds-bot-types";
...
function defaultDataForType(type: DsBotNodeType): Record<string, unknown> {
  switch (type) {
    case "trigger":              return { label: "Início" };
    case "bubble_text":          return { text: "Escreva aqui..." };
    case "bubble_image":         return { url: "" };
    case "bubble_video":         return { url: "" };
    case "bubble_audio":         return { url: "" };
    case "bubble_embed":         return { url: "" };
    case "input_text":           return { question: "Qual sua resposta?", variable: "resposta" };
    case "input_number":         return { question: "Informe um número", variable: "numero" };
    case "input_email":          return { question: "Qual seu e-mail?", variable: "email" };
    case "input_website":        return { question: "Qual URL?", variable: "url" };
    case "input_date":           return { question: "Escolha uma data", variable: "data" };
    case "input_phone":          return { question: "Qual seu telefone?", variable: "telefone" };
    case "input_button":         return { question: "Escolha uma opção", variable: "opcao", options: [{ id: "opt-0", label: "Sim", value: "yes" }, { id: "opt-1", label: "Não", value: "no" }] };
    case "input_file":           return { question: "Envie o arquivo", variable: "arquivo", accept: "image/*,application/pdf", max_mb: 10 };
    case "flow_goto":            return { target_node_id: "" };
    case "flow_back":            return {};
    case "flow_end":             return { reason: "" };
    case "flow_wait":            return { timeout_seconds: 60 };
    case "contact_add_tag":      return { tag: "" };
    case "contact_remove_tag":   return { tag: "" };
    case "contact_update_field": return { field: "", value: "" };
    case "message_waba_template":return { template_id: "" };
    case "message_ds_voice":     return { library_item_id: "" };
    case "message_forward":      return { message_id: "" };
    case "attendance_transfer_queue": return { queue_id: "" };
    case "attendance_assign_agent":   return { agent_id: "" };
    case "attendance_open_protocol":  return { subject: "", priority: "normal" };
    case "attendance_close":     return {};
    case "conditional":          return { logic: "AND", clauses: [{ left: "var.x", op: "eq", right: "" }] };
    case "agent_handoff":        return { agent_id: "", context_vars: [] };
    default:                     return {};
  }
}
...
function toRFEdges(edges: DsBotEdge[]): Edge[] {
  return edges.map((e) => ({ id: e.id, source: e.source, target: e.target, sourceHandle: e.sourceHandle ?? undefined, targetHandle: e.targetHandle ?? undefined, label: e.label }));
}

function Inner({ initial, onChange, onSelectionChange }: Props) {
  const [nodes, setNodes, onNodesChange] = useNodesState<Node>(toRFNodes(initial.nodes));
  const [edges, setEdges, onEdgesChange] = useEdgesState<Edge>(toRFEdges(initial.edges));
  ...
  const emit = useCallback((ns: Node[], es: Edge[]) => {
    const dsNodes: DsBotNode[] = ns.map((n) => {
      ...
      const category = existing?.category ?? inferCategory(n.type as DsBotNodeType);
      return {
        id: n.id,
    ...
    onChange({ nodes: dsNodes, edges: dsEdges, viewport: reactFlow.getViewport() });
  ...
  const onDrop = useCallback((event: React.DragEvent) => {
    ...
    const type = event.dataTransfer.getData("application/dsbot-node-type") as DsBotNodeType;
    if (!type) return;
  ...
  useMemo(() => { onSelectionChange?.(selectedNode); }, [selectedNode, onSelectionChange]);
