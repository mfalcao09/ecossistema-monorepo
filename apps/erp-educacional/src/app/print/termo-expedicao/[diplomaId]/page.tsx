...
export default function PrintTermoExpedicaoPage({
  ...
  const nomeDiplomado =
    snap?.diplomado.nome ?? data.dadosAluno?.nome ?? '—'
  const cpfDiplomado =
    snap?.diplomado.cpf ?? data.dadosAluno?.cpf ?? '—'
  const cursoNome =
    snap?.curso.nome ?? data.dadosCurso?.curso_nome ?? '—'
  ...
  const dataColacao =
    snap?.dados_academicos.data_colacao_grau ?? data.dadosCurso?.data_colacao ?? null
  const dataConclusao =
    snap?.dados_academicos.data_conclusao ?? data.dadosCurso?.data_conclusao ?? null
  const dataExpedicao =
    snap?.dados_academicos.data_expedicao ?? data.dadosCurso?.data_expedicao ?? null
  ...
  const processo = snap?.registro?.processo ?? data.dadosCurso?.processo ?? null

  const iesNome =
    snap?.ies_emissora?.nome ?? cfg?.historico_texto_rodape ?? 'Faculdades Integradas de Cassilândia'
  const iesMunicipio =
    snap?.ies_emissora?.municipio ?? 'Cassilândia'
  ...
  const dadosTermo: DadosTermoExpedicao = {
    ...
    folha,
    processo,
    ies_nome: typeof iesNome === 'string' ? iesNome : 'Faculdades Integradas de Cassilândia',
  ...
  const timbradoUrl =
    timbradoRaw && !timbradoRaw.toLowerCase().endsWith('.pdf') ? timbradoRaw : null
