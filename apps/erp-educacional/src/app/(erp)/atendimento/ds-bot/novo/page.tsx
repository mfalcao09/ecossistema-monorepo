...
export default function NewBotPage() {
  ...
  const [triggerType, setTriggerType] = useState<"keyword" | "tag_added" | "new_conversation" | "manual">("manual");
  ...
  useEffect(() => {
    fetch("/api/atendimento/ds-bots").then((r) => r.json()).then((j) => setTemplates(j.templates ?? []));
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (mode === "template") {
        if (!templateSlug) { setError("Escolha um template."); setSaving(false); return; }
      ...
      if (mode === "import") {
        try {
          ...
          setError("JSON inválido."); setSaving(false); return;
        }
      }
      ...
      if (!res.ok) { setError(j.error || "Falha ao criar."); setSaving(false); return; }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Link href="/atendimento/ds-bot" className="inline-flex items-center gap-1 text-sm text-gray-600 hover:text-gray-900 mb-3">
      ...
      <div className="bg-white border rounded-lg p-5 space-y-4">
        ...
        {mode !== "template" && mode !== "import" && (
          <>
            <Field label="Gatilho">
              <select value={triggerType} onChange={(e) => setTriggerType(e.target.value as typeof triggerType)} className="w-full border rounded px-3 py-2">
            ...
            {(triggerType === "keyword" || triggerType === "tag_added") && (
              <Field label={triggerType === "keyword" ? "Palavra-chave" : "Nome da tag"}>
                <input value={triggerValue} onChange={(e) => setTriggerValue(e.target.value)} className="w-full border rounded px-3 py-2" />
              </Field>
            )}
            <Field label="Canais">
              <div className="flex gap-2 flex-wrap">
                {["whatsapp", "instagram", "facebook"].map((c) => (
                  <label key={c} className="flex items-center gap-1 text-sm border rounded px-2 py-1 cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {templates.map((t) => (
                <button
                  key={t.slug}
                  type="button"
                  onClick={() => { setTemplateSlug(t.slug); setName(t.name); setDescription(t.description); }}
            ...
            <label className="mt-2 inline-flex items-center gap-1 text-xs text-violet-600 cursor-pointer">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  reader.onload = (ev) => setImportPayload(String(ev.target?.result ?? ""));
                  reader.readAsText(f);
                }}
              />
            </label>
        ...
        <div className="flex justify-end gap-2 pt-2 border-t">
          <Link href="/atendimento/ds-bot" className="px-4 py-2 text-sm border rounded hover:bg-gray-50">Cancelar</Link>
          <button onClick={submit} disabled={saving} className="px-4 py-2 text-sm bg-violet-600 text-white rounded disabled:opacity-50 hover:bg-violet-700">
