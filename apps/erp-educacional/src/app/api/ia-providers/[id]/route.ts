...
export const PATCH = protegerRota(
  async (request: NextRequest) => {
    ...
    const allowedFields = ['nome', 'slug', 'base_url', 'api_key', 'formato_api', 'headers_extras', 'ativo', 'ordem']
...
export const DELETE = protegerRota(
  async (request: NextRequest) => {
    ...
    const { error } = await admin.from('ia_providers').delete().eq('id', providerId)
    if (error) {
      return NextResponse.json({ erro: sanitizarErro(error.message, 500) }, { status: 500 })
    }
