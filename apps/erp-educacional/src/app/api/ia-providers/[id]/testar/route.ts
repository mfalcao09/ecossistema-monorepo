...
async function testarOpenAICompatible(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { ok: false, erro: `HTTP ${res.status}: ${txt.substring(0, 200)}` }
    }

    const data = await res.json()
    const modelos: ModeloItem[] = (data.data ?? []).map((m: { id: string; name?: string }) => ({
      id: m.id,
      name: m.name || m.id,
    }))
...
export const POST = protegerRota(
  async (request: NextRequest) => {
    ...
    if (!providerId) {
      return NextResponse.json({ erro: 'ID do provider é obrigatório' }, { status: 400 })
    }
    ...
    if (fetchError || !provider) {
      return NextResponse.json({ erro: 'Provider não encontrado' }, { status: 404 })
    }

    if (!provider.api_key) {
      return NextResponse.json({ ok: false, erro: 'Provider sem chave de API configurada' }, { status: 400 })
    }
    ...
    switch (provider.formato_api) {
      case 'openai_compatible':
        resultado = await testarOpenAICompatible(provider.base_url, provider.api_key, provider.headers_extras)
        break
      case 'google_genai':
        resultado = await testarGoogleGenAI(provider.base_url, provider.api_key)
        break
      case 'anthropic_messages':
        resultado = await testarAnthropicMessages(provider.base_url, provider.api_key, provider.headers_extras)
        break
      default:
        return NextResponse.json({ ok: false, erro: `Formato "${provider.formato_api}" não suportado` }, { status: 400 })
    }
