...
export async function GET(
  ...
  try {
    ...
    //
    // Acesso compartilhado (2026-04-23, decisão Marcelo):
    //   Removido `.eq('usuario_id', userId)` — uma extração iniciada por
    //   um membro do tenant precisa ser acessível por outros membros
    //   (controle correto deve ser por tenant + RBAC do módulo Diploma,
    //   não por dono individual). Hoje há 2 usuários compartilhando
    //   responsabilidade no piloto FIC.
    //
    // Defesas que continuam:
    //   - protegerRota / verificarAuth (auth obrigatória)
    //   - RLS `auth.uid() IS NOT NULL` (qualquer authenticated)
    //
    // Follow-up estrutural pendente:
    //   - Adicionar coluna `tenant_id` em `extracao_sessoes` + popular
    //   - RLS por tenant (`tenant_id = current_setting('app.tenant_id')`)
    //   - Verificar permissão RBAC `diploma:editar` no handler
    ...
    if (arquivosResult.error) {
      console.error(
        "[GET sessoes] arquivos err:",
        arquivosResult.error.message,
      );
    }
...
export async function PUT(
  ...
  // 4. Valida que a sessão existe + está em status editável
  // Acesso compartilhado entre usuários do tenant (ver comentário no GET).
      ...
      return NextResponse.json(
        {
          ...
          mensagem:
            "O processo possui diploma(s) publicado(s) ou assinado(s). A revisão está bloqueada.",
        },
        { status: 403 },
      );
    }
