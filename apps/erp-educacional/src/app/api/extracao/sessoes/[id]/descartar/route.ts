...
export async function POST(
  ...
  if (selectErr) {
    console.error(
      "[extracao/descartar] Erro ao buscar sessão:",
      selectErr.message,
    );
  ...
  // Acesso compartilhado entre usuários do tenant (decisão Marcelo 2026-04-23):
  // Removido check `sessao.usuario_id !== auth.userId` — qualquer membro
  // do módulo Diploma pode descartar sessões. Auditoria fica via timestamp
  // `finalizado_em` (quando) e tabela `auth_attempts` (quem). Follow-up:
  // adicionar `descartado_por` na sessão pra rastrear ação específica.

  // Idempotência: já está descartado, retorna OK.
  if (sessao.status === "descartado") {
    return NextResponse.json(
      { ok: true, ja_descartado: true },
      { status: 200 },
    );
  }
