...
export const GET = protegerRota(
  async (request) => {
    ...
    const config =
      configs?.find((c: any) =>
        isTimbradoValido(c.historico_arquivo_timbrado_url),
      ) ??
      configs?.[0] ??
      null;
    ...
    const dadosAluno = {
      ...
      naturalidade: [
        diplomado?.naturalidade_municipio,
        diplomado?.naturalidade_uf,
      ]
        .filter(Boolean)
        .join("/"),
    };
    ...
    const reconhecimento = curso?.numero_reconhecimento
      ? `${curso.tipo_reconhecimento ?? "Portaria"} nº ${curso.numero_reconhecimento}${
          curso.data_reconhecimento
            ? ` de ${formatDateBR(curso.data_reconhecimento)}`
            : ""
    ...
    const renovacao = curso?.numero_renovacao
      ? `${curso.tipo_renovacao ?? "Portaria"} nº ${curso.numero_renovacao}${
          curso.data_renovacao
            ? ` de ${formatDateBR(curso.data_renovacao)}`
            : ""
        }`
      : "";
    ...
    const dadosCurso = {
      ...
      grau: curso?.grau ?? "",
      titulo_conferido:
        diploma.titulo_conferido ?? curso?.titulo_conferido ?? "",
