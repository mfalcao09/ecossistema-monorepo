...
export async function GET(
  ...
  const { data: arquivos, error: arquivosErr } = await supabase
    ...
    .select(
      "id, nome_original, tipo_documento, descricao_ia, storage_path, mime_type, tamanho_bytes, created_at",
    )
    .eq("processo_id", id)
...
export async function POST(
  ...
  if (!file || typeof file === "string") {
    return NextResponse.json(
      { error: 'Campo "arquivo" é obrigatório' },
      { status: 400 },
    );
  }
  ...
  const tipoDetectado = await detectarTipoDocumento(
    file.name,
    file.type,
    bufferBase64,
  );
...
export async function DELETE(
  ...
  if (!arquivoId) {
    return NextResponse.json(
      { error: 'Parâmetro "arquivo_id" é obrigatório' },
      { status: 400 },
    );
  }
