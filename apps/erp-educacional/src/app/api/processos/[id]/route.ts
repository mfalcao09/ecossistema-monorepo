...
export async function GET(
  ...
  const diplomaId = diplomasFormatted.length > 0 ? diplomasFormatted[0].id : null;
...
export async function PATCH(
  ...
  const allowedFields = ["nome", "status", "obs", "turno", "periodo_letivo", "data_colacao"];
...
export async function DELETE(
  ...
  if (fetchError) {
    console.error('[API] Erro ao buscar processo para delete:', fetchError.message);
    return erroInterno();
  }
