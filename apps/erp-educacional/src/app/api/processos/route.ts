...
async function rollback(
  ...
  ids: { processo_id?: string; diploma_id?: string; diplomado_id?: string; diplomado_era_novo?: boolean }
) {
  const erros: string[] = []
  ...
  if (ids.diplomado_id && ids.diplomado_era_novo) {
    const { error: errFil } = await supabase.from('filiacoes').delete().eq('diplomado_id', ids.diplomado_id)
  ...
  if (ids.diploma_id) {
    ...
    for (const tabela of tabelas) {
      const { error } = await supabase.from(tabela).delete().eq('diploma_id', ids.diploma_id)
      if (error) erros.push(`rollback ${tabela}: ${error.message}`)
    }
    const { error } = await supabase.from('diplomas').delete().eq('id', ids.diploma_id)
    if (error) erros.push(`rollback diplomas: ${error.message}`)
  }

  // Só deleta diplomado se foi criado nesta operação (não se já existia)
  if (ids.diplomado_id && ids.diplomado_era_novo) {
    const { error } = await supabase.from('diplomados').delete().eq('id', ids.diplomado_id)
    if (error) erros.push(`rollback diplomados: ${error.message}`)
  }

  if (ids.processo_id) {
    const { error } = await supabase.from('processos_emissao').delete().eq('id', ids.processo_id)
  ...
  async (request, { userId, tenantId }) => {
    ...
    try {
      ...
      if (errProcesso || !processo) {
        return NextResponse.json(
          { error: sanitizarErro(errProcesso?.message || 'Erro ao criar processo', 500) },
          { status: 500 }
        )
      }
      ...
      const dadosDiplomado = {
        ...
        codigo_municipio_ibge: dados_diplomado.naturalidade_codigo_municipio || null,
      }
      ...
      if (existente) {
        ...
        if (errUpdate) {
          console.error('[API] Erro ao atualizar diplomado:', errUpdate.message)
          await rollback(supabase, criados)
          return NextResponse.json(
        ...
        const { data: novo, error: errDiplomado } = await supabase
          ...
          .insert({
            ...dadosDiplomado,
            cpf: cpfLimpo,
            data_nascimento: dados_diplomado.data_nascimento || new Date().toISOString().split('T')[0],
          })
          .select('id')
          .single()

        if (errDiplomado || !novo) {
          console.error('[API] Erro ao criar diplomado:', errDiplomado?.message)
          await rollback(supabase, criados)
          return NextResponse.json(
      ...
      if (Array.isArray(disciplinas) && disciplinas.length > 0) {
        const situacoesValidas = ['aprovado', 'reprovado', 'trancado', 'cursando', 'aproveitado', 'dispensado']
        ...
        const rows = disciplinas.map((d: any, idx: number) => {
          ...
          return {
            ...
            carga_horaria_aula: d.carga_horaria ? parseInt(d.carga_horaria) || null : null,
            carga_horaria_relogio: d.ch_hora_relogio ? parseInt(d.ch_hora_relogio) || null : null,
            nota: d.nota ? parseFloat(d.nota) || null : null,
            nota_ate_cem: d.nota_ate_100 ? parseFloat(d.nota_ate_100) || null : null,
      ...
      // ═══════════════════════════════════════════════════════════
      if (Array.isArray(atividades_complementares) && atividades_complementares.length > 0) {
        const rows = atividades_complementares.map((a: any) => ({
          diploma_id: criados.diploma_id,
          descricao: a.descricao || a.nome || 'Sem descrição',
          carga_horaria_relogio: a.carga_horaria ? parseInt(a.carga_horaria) || null : null,
          tipo: a.tipo || null,
        ...
        if (errAtiv) {
          console.error('[API] Erro ao inserir atividades complementares:', errAtiv.message)
          await rollback(supabase, criados)
          return NextResponse.json(
      ...
      if (Array.isArray(estagios) && estagios.length > 0) {
        const rows = estagios.map((e: any) => ({
          ...
          carga_horaria_relogio: e.carga_horaria ? parseInt(e.carga_horaria) || null : null,
      ...
      if (Array.isArray(genitores) && genitores.length > 0) {
        ...
        if (!criados.diplomado_era_novo) {
          await supabase.from('filiacoes').delete().eq('diplomado_id', criados.diplomado_id!)
        }
      ...
      if (errTotal) {
        console.error('[API] Erro ao atualizar total_diplomas:', errTotal.message)
