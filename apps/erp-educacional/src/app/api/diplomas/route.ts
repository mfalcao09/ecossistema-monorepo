...
// Next.js 15 + Fluid Compute precisa de `dynamic = 'force-dynamic'` em rotas
// que usam cookies() via SSR client. Sem isso, runtime trava em cold-start.
// `maxDuration=20` força fail-fast.
...
export const GET = protegerRota(
  async (request: NextRequest) => {
    const t0 = Date.now();
    console.log("[api/diplomas] enter handler");
    const supabase = await createClient();
    console.log("[api/diplomas] supabase client ready", {
      dt: Date.now() - t0,
    });
