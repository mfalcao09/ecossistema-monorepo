...
export async function GET(
  ...
  if (d.processo_id) {
    const { data: procRaw } = await adminPre
      ...
      .select(
        "id, nome, status, curso_id, turno, periodo_letivo, data_colacao, total_diplomas, created_at",
      )
      .eq("id", d.processo_id)
      .maybeSingle();
  ...
  const { data: xmlsRaw } = await admin
    .from("xml_gerados")
    .select(
      "id, tipo, status, validado_xsd, erros_validacao, arquivo_url, hash_sha256, created_at",
    )
    .eq("diploma_id", id)
  ...
  if (d.processo_id) {
    const { data: extRaw } = await admin
      ...
      .select(
        "id, status, confianca_geral, campos_faltando, dados_extraidos, dados_confirmados, created_at",
      )
      ...
      .maybeSingle();
    if (extRaw)
      extracaoSessao = extRaw as unknown as NonNullable<typeof extracaoSessao>;
  }

  // ── Fluxo de assinaturas com dados do assinante ──────────────────────────
  const fluxoAssinaturas = (completo.fluxo_assinaturas ?? []).map((f) => {
    const assinante = (completo.assinantes ?? []).find(
      (a) => a.id === f.assinante_id,
    );
    return {
      id: f.id,
      ordem: f.ordem,
...
export async function DELETE(
  ...
  if (fetchError) {
    console.error(
      "[API] Erro ao buscar diploma para delete:",
      fetchError.message,
    );
    return erroInterno();
  }
