...
async function buscarConfigIes(
  ...
  const { data, error } = await admin
    .from("diploma_config")
    .select(
      "ies_nome, ies_sigla, ies_cnpj, municipio, uf, url_portal_diplomatizado",
    )
...
export async function GET(
  ...
  const { data: diploma, error } = await admin
    .from("diplomas")
    .select(
      "id, status, url_verificacao, qrcode_url, codigo_validacao, ambiente",
    )
    .eq("id", id)
    .single();
  ...
  const urlVerificacao =
    diploma.url_verificacao ?? `${baseUrl}/verificar/${codigoVerificacao}`;
...
export async function POST(
  ...
  try {
    ...
    if (!configIes) {
      console.error(
        "[API] Configuração da IES não encontrada para ambiente:",
    ...
    try {
      ...
      erroVariaveis =
        err instanceof Error ? err.message : "Erro ao montar variáveis";
      console.warn(
        "[POST /rvdd] Aviso — variáveis incompletas:",
        erroVariaveis,
      );
