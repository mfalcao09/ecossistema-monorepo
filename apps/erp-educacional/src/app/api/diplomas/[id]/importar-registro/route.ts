  ...
  try {
    ...
    if (!xml_content || typeof xml_content !== "string") {
      return NextResponse.json(
        { error: "xml_content é obrigatório e deve ser uma string com o conteúdo do XML." },
        { status: 400 }
      )
    }
    ...
    const statusPermitidos = ["aguardando_assinatura", "assinado_emissora", "enviado_registradora", "aguardando_registro"]
    ...
    if (xmlInsertError) {
      ...
      return NextResponse.json({ error: "Erro ao salvar XML no banco." }, { status: 500 })
    }
    ...
    if (dados_registro.livro_registro) updateDiploma.livro_registro = dados_registro.livro_registro
    if (dados_registro.numero_registro) updateDiploma.numero_registro = dados_registro.numero_registro
    if (dados_registro.processo_sei) updateDiploma.processo_sei = dados_registro.processo_sei
    if (dados_registro.data_registro) updateDiploma.data_registro = dados_registro.data_registro
    if (dados_registro.codigo_validacao) updateDiploma.codigo_validacao = dados_registro.codigo_validacao
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: "Erro ao atualizar diploma com dados de registro." }, { status: 500 })
    }
      ...
      if (fluxoExistente && fluxoExistente.length > 0) {
        for (const fluxo of fluxoExistente) {
          ...
          await supabase
            ...
            .update({
              status: "assinado",
              data_assinatura: assinaturaCorrespondente?.data_assinatura ?? new Date().toISOString(),
              updated_at: new Date().toISOString(),
            })
            .eq("id", fluxo.id)
        }
    ...
    return NextResponse.json({
      ...
      mensagem: "XML do DiplomaDigital importado com sucesso. Diploma atualizado para status 'registrado'.",
    }, { status: 200 })
