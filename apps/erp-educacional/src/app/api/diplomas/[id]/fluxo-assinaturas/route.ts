...
export async function GET(req: NextRequest) {
  const handler = protegerRota(async (request) => {
    ...
    if (!diplomaId) {
      return NextResponse.json({ error: 'diploma_id ausente' }, { status: 400 })
    }
...
export async function POST(req: NextRequest) {
  const handler = protegerRota(async (request) => {
    ...
    if (!assinante_id || typeof assinante_id !== 'string') {
      return NextResponse.json({ error: 'assinante_id obrigatório' }, { status: 400 })
    }
    ...
    if (errDip || !diploma) {
      return NextResponse.json(
        { error: sanitizarErro(errDip?.message || 'Diploma não encontrado', 404) },
        { status: 404 }
      )
    }
