...
export const POST = protegerRota(
  async (request, { userId }) => {
    ...
    const statusPermitidos = ['aguardando_digitalizacao', 'documentos_assinados', 'acervo_completo', 'erro']
    ...
    if (!tipo) {
      return NextResponse.json({ error: 'Tipo do documento é obrigatório' }, { status: 400 })
    }
    ...
    try {
      ...
      if (diploma.status === 'documentos_assinados') {
        await supabase
          .from('diplomas')
          .update({ status: 'aguardando_digitalizacao', updated_at: new Date().toISOString() })
          .eq('id', diplomaId)
      }
  ...
  async (request, { userId }) => {
    ...
    if (body.acao === 'finalizar') {
      ...
      if (!docs || docs.length === 0) {
        return NextResponse.json({
          error: 'Nenhum documento no acervo. Adicione pelo menos um documento antes de finalizar.',
        }, { status: 422 })
      }

      const { error } = await supabase
        .from('diplomas')
        .update({ status: 'acervo_completo', updated_at: new Date().toISOString() })
        .eq('id', diplomaId)
