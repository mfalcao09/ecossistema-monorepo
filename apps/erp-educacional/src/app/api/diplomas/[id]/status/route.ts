...
async function verificarChecklist(
  ...
  switch (statusDesejado) {
    ...
    case "aguardando_documentos": {
      ...
      const xmlsAssinados = (xmls || []).filter(
        (x: any) => x.status === "assinado",
      );
    ...
    case "aguardando_digitalizacao": {
      ...
      const docsAssinados = (docs || []).filter(
        (d: any) => d.status === "assinado",
      );
      if (docsAssinados.length < 3) {
        pendencias.push(
          `Apenas ${docsAssinados.length}/3 documentos complementares assinados`,
        );
      }
      break;
    }
    ...
    case "aguardando_envio_registradora": {
      ...
      if (
        acervoDocs &&
        acervoDocs.length > 0 &&
        acervoAssinados.length < acervoDocs.length
      ) {
        pendencias.push(
          `${acervoAssinados.length}/${acervoDocs.length} documentos do acervo assinados`,
        );
      }
      break;
    }
  ...
  async (request, { userId }) => {
    ...
    if (errBusca || !diploma) {
      return NextResponse.json(
        {
          error: sanitizarErro(
            errBusca?.message || "Diploma não encontrado",
            404,
          ),
        },
        { status: 404 },
    ...
    const { aprovado, pendencias } = await verificarChecklist(
      supabase,
      diplomaId,
      novoStatus,
    );
    ...
    supabase
      ...
      .then((res) => {
        if (res.error)
          console.error("[API] Erro ao registrar log de transição:", res.error);
      });
...
export const GET = protegerRota(
  async (request) => {
    ...
    const checklistPorTransicao: Record<
      string,
      { aprovado: boolean; pendencias: string[] }
    > = {};
    for (const possivel of transicoesPermitidas) {
      checklistPorTransicao[possivel] = await verificarChecklist(
        supabase,
        diplomaId,
        possivel,
      );
