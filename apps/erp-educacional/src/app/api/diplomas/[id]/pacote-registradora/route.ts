  ...
  async (request, { userId }) => {
    ...
    try {
      ...
      const { data: xmls } = await supabase
        .from('xml_gerados')
        .select('tipo, conteudo_xml, hash_sha256, arquivo_url, status, carimbo_tempo_base64, carimbo_tempo_nonce, carimbo_tempo_at')
      ...
      const xmlsCarimbados = (xmls || []).filter((x: any) => x.carimbo_tempo_base64)
      const xmlsSemCarimbo = (xmls || []).filter((x: any) => !x.carimbo_tempo_base64)
      ...
      const manifest = {
        ...
        aviso: xmlsSemCarimbo.length > 0
          ? `${xmlsSemCarimbo.length} XML(s) sem carimbo do tempo: ${xmlsSemCarimbo.map((x: any) => x.tipo).join(', ')}`
          : null,
        total_arquivos: (xmls?.length ?? 0) * 2 + (docsComplementares?.length ?? 0) + (docsAcervo?.length ?? 0) + 1,
      }
      ...
      for (const xml of (xmls || [])) {
        ...
        if ((xml as any).arquivo_url) {
          try {
            const resp = await fetch((xml as any).arquivo_url, { signal: AbortSignal.timeout(10_000) })
            if (resp.ok) {
            ...
            if (xml.conteudo_xml) archive.append(xml.conteudo_xml, { name: nomeArquivo })
          }
        } else if (xml.conteudo_xml) {
          archive.append(xml.conteudo_xml, { name: nomeArquivo })
        }
        ...
        if ((xml as any).carimbo_tempo_base64) {
          const carimboBuffer = Buffer.from((xml as any).carimbo_tempo_base64, 'base64')
          archive.append(carimboBuffer, { name: `xmls/${xml.tipo}_carimbo.p7s` })
        }
      }
      ...
      for (const doc of (docsComplementares || [])) {
        ...
        const urlDoc = (doc as any).arquivo_assinado_url ?? (doc as any).arquivo_url
        if (!urlDoc) continue
        try {
          const resp = await fetch(urlDoc, { signal: AbortSignal.timeout(10_000) })
          if (resp.ok) {
      ...
      const cpfDiplomado = (diploma.diplomados as any)?.cpf?.replace(/\D/g, '').slice(0, 11) ?? diplomaId.slice(0, 8)
