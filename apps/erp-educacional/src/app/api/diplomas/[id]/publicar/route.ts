...
export async function POST(
  ...
  if (diploma.status === "publicado") {
    return NextResponse.json(
      { error: "Diploma já está publicado." },
      { status: 400 },
    );
  }
  ...
  if (!STATUS_PUBLICAVEIS.includes(diploma.status)) {
    return NextResponse.json(
      {
        error:
          `Diploma não pode ser publicado no status "${diploma.status}". ` +
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_APP_URL ?? "https://diploma.ficcassilandia.com.br";
  ...
  const urlVerificacao =
    (diploma.url_verificacao as string | null) ??
