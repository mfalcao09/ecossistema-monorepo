...
import {
  montarVariaveisRVDD,
  type ConfigIesParaRVDD,
} from "@/lib/diplomas/montar-variaveis-rvdd";
import {
  verificarAuth,
  erroNaoEncontrado,
  erroInterno,
...
export async function GET(
  ...
  if (!configIes) {
    console.error(
      "[API] Configuração da IES não encontrada para o ambiente:",
      completo.diploma.ambiente,
    );
    return erroInterno();
  }
  ...
  try {
    ...
    erroVariaveis =
      err instanceof Error ? err.message : "Erro ao montar variáveis RVDD";
...
async function buscarConfigIes(
  ambiente: string,
): Promise<ConfigIesParaRVDD | null> {
  ...
  const urlPortal =
    ((data as Record<string, unknown>).url_portal_diplomatizado as
      | string
      | null) ??
  ...
  return {
    nome:
      ((data as Record<string, unknown>).ies_nome as string) ??
      "Faculdades Integradas de Cassilândia",
    sigla:
      ((data as Record<string, unknown>).ies_sigla as string | null) ?? "FIC",
    cnpj: ((data as Record<string, unknown>).ies_cnpj as string) ?? "",
    municipio:
      ((data as Record<string, unknown>).municipio as string) ?? "Cassilândia",
