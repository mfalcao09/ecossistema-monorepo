...
export const GET = protegerRota(async (request) => {
  ...
  const { data: edicoes } = await supabase
    ...
    .select('id, usuario_id, justificativa, campos_alterados, versao_antes, versao_depois, created_at')
...
export const PATCH = protegerRota(async (request, { userId }) => {
  ...
  if (!diploma.dados_snapshot_extracao) {
    return NextResponse.json(
      { error: 'Diploma sem snapshot — não pode ser editado (possivelmente legado)' },
      { status: 422 }
    )
  }
  ...
  if (errAudit) {
    ...
    return NextResponse.json({
      ...
      versao: versaoDepois,
      diff,
      alerta: 'Snapshot atualizado mas auditoria falhou — contate administrador',
    })
  }
