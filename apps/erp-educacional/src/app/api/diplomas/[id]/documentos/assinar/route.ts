  ...
  async (request) => {
    ...
    if (!bryConfig) {
      return NextResponse.json(
        { error: 'Integração BRy não configurada. Verifique BRY_CLIENT_ID e BRY_CLIENT_SECRET.' },
        { status: 503 }
      )
    }
    ...
    if (errDocs || !docs || docs.length === 0) {
      return NextResponse.json(
        { error: 'Nenhum documento gerado encontrado. Execute POST /documentos primeiro.' },
        { status: 422 }
      )
    }
    ...
    const signatarios = ((fluxo as FluxoRow[] | null) ?? [])
      .filter(f => f.assinantes?.nome && f.assinantes?.cpf && f.assinantes?.email)
      .map(f => ({
    ...
    if (signatarios.length === 0) {
      return NextResponse.json(
        {
          error: 'Nenhum signatário com nome, CPF e e-mail cadastrado para este diploma. ' +
    ...
    const diplomadoNome = (diploma.diplomados as { nome?: string } | null)?.nome ?? 'Diplomado'
    const resultados: Array<{ tipo: string; documentoId: string; ok: boolean; erro?: string }> = []
    ...
    for (const doc of docs) {
      try {
        ...
        if (doc.arquivo_path) {
          ...
          if (errFile || !fileData) {
            throw new Error(`Erro ao baixar PDF do storage: ${errFile?.message ?? 'arquivo não encontrado'}`)
          }
          ...
          const res = await fetch(doc.arquivo_url, { signal: AbortSignal.timeout(20_000) })
          if (!res.ok) throw new Error(`Erro ao baixar PDF via URL: ${res.status}`)
        ...
        resultados.push({ tipo: doc.tipo, documentoId: resultado.documentoId, ok: true })
        ...
        resultados.push({ tipo: doc.tipo, documentoId: '', ok: false, erro: msg.slice(0, 200) })
      }
    }
