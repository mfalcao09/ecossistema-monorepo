  ...
  async (request, { userId }) => {
    ...
    try {
      ...
      const dadosHistorico: DadosHistoricoPDF = {
        ...
        turno: diploma.turno,
        carga_horaria_total:
          diploma.carga_horaria_integralizada ?? curso.carga_horaria_total,
      ...
      const temSnapshot = Boolean(
        (diploma as { dados_snapshot_extracao?: unknown })
      ...
      if (temSnapshot) {
        ...
        const { origin, cookieDomain, cookieSecure } =
          derivePrintContext(request);
      ...
      if (errosUpload.length > 0) {
        console.error(
          "[API] Erros de upload:",
          errosUpload.map((u) => u.error?.message),
        );
      }
...
export const GET = protegerRota(
  async (request) => {
    ...
    const { data: docs, error } = await supabase
      .from("diploma_documentos_complementares")
      .select(
        "id, tipo, status, arquivo_url, arquivo_assinado_url, bry_document_id, gerado_em, assinado_em, created_at",
      )
      .eq("diploma_id", diplomaId)
