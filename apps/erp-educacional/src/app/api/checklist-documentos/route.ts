...
export const GET = protegerRota(async (req: NextRequest, { userId, tenantId }) => {
  try {
    ...
    const tiposVinculo = tipoVinculoParam.split(',').map(t => t.trim()).filter(Boolean)
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          erro:
            sanitizarErro('Failed to create checklist item. Database error.', 500),
        },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json(
      { erro: sanitizarErro('Internal server error creating checklist item', 500) },
      { status: 500 }
    )
  }
})
