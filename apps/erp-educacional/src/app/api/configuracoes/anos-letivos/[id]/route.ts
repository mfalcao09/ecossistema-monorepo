...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Erro ao buscar ano letivo',
      },
      { status: 500 }
    )
  }
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Erro ao atualizar ano letivo',
      },
      { status: 500 }
    )
  }
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('violates foreign key')) {
      return NextResponse.json(
        {
          error: 'Não é possível excluir este ano letivo pois possui períodos letivos ou eventos de calendário associados',
        },
        { status: 409 }
      )
    }
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Erro ao excluir ano letivo',
      },
      { status: 500 }
    )
  }
