...
export const GET = protegerRota(
  async (_req: NextRequest, _ctx) => {
    ...
    if (error) {
      ...
      return NextResponse.json({ erro: 'Erro ao buscar pipelines' }, { status: 500 })
    }
...
export const POST = protegerRota(
  async (req: NextRequest, _ctx) => {
    ...
    if (!body.key || !body.name) {
      return NextResponse.json({ erro: 'key e name obrigatórios' }, { status: 400 })
    }
    ...
    if (body.stages?.length) {
      ...
      const { error: errS } = await supabase.from('pipeline_stages').insert(payload)
      if (errS) {
        console.error('[POST pipelines stages]', errS)
        return NextResponse.json({ erro: 'Pipeline criado, mas stages falharam', pipeline }, { status: 207 })
      }
    }
