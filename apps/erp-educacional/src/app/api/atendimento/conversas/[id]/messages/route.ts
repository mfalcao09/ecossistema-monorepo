  ...
  async (request: NextRequest, { userId }) => {
    ...
    if (!content) {
      return NextResponse.json(
        { erro: "Conteúdo da mensagem é obrigatório" },
        { status: 400 },
      );
    }
    ...
    const { data: msgSalva, error: errSave } = await supabase
      ...
      .select(
        "id, content, message_type, content_type, status, created_at, sender_type",
      )
      .single();
    ...
    const destinatario = (conv.atendimento_contacts as { phone_number: string })
      ?.phone_number;
