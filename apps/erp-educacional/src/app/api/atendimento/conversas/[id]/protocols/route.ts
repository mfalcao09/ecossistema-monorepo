...
export const GET = protegerRota(
  async (req: NextRequest, _ctx) => {
    ...
    if (error) {
      return NextResponse.json({ erro: 'Erro ao buscar protocolos' }, { status: 500 })
    }
...
export const POST = protegerRota(
  async (req: NextRequest, ctx) => {
    ...
    if (error || !data) {
      ...
      return NextResponse.json({ erro: 'Erro ao criar protocolo' }, { status: 500 })
    }
...
export const PATCH = protegerRota(
  async (req: NextRequest, _ctx) => {
    ...
    if (!protocolId) {
      return NextResponse.json({ erro: 'protocol_id obrigatório' }, { status: 400 })
    }
    ...
    if (error) {
      return NextResponse.json({ erro: 'Erro ao atualizar protocolo' }, { status: 500 })
    }
