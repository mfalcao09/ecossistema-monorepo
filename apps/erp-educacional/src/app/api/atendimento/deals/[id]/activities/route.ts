...
export const GET = protegerRota(
  async (req: NextRequest, _ctx) => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, type, title, description, scheduled_at, duration_minutes, assignee_id, completed_at, attachment_url, created_by, created_at",
      )
      .eq("deal_id", dealId)
...
export const POST = protegerRota(
  async (req: NextRequest, ctx) => {
    ...
    if (!body.type || !body.title) {
      return NextResponse.json(
        { erro: "type e title obrigatórios" },
        { status: 400 },
      );
    }
