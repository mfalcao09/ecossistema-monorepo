...
const createSchema = z.object({
  ...
  trigger_type: z
    .enum(["keyword", "tag_added", "new_conversation", "manual"])
    .default("manual"),
...
export async function GET(request: NextRequest) {
  ...
  let query = admin
    .from("ds_bots")
    .select(
      "id, name, description, trigger_type, trigger_value, channels, enabled, version, created_at, updated_at",
      { count: "exact" },
    )
...
export async function POST(request: NextRequest) {
  ...
  if (parsed.data.source === "template") {
    ...
    if (!parsed.data.flow)
      return NextResponse.json(
        { error: "flow_required_for_import" },
        { status: 400 },
      );
  ...
  await admin.from("ds_bot_versions").insert({
    ...
    flow_json: flow,
    change_note:
      parsed.data.source === "template"
        ? `from:${parsed.data.template_slug}`
        : parsed.data.source,
    created_by: user.id,
  });
