...
export async function GET(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  let query = admin
    ...
    .select("id, bot_id, version, conversation_id, contact_id, channel, current_node_id, awaiting_input, variables, history, status, started_at, updated_at, completed_at, error", { count: "exact" })
    .eq("bot_id", id)
