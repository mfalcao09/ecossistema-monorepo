...
function validateFlow(flow: DsBotFlow, start_node_id: string | null): string[] {
  ...
  for (const edge of flow.edges) {
    if (!ids.has(edge.source))
      issues.push(`Edge ${edge.id}: source ${edge.source} não existe`);
    if (!ids.has(edge.target))
      issues.push(`Edge ${edge.id}: target ${edge.target} não existe`);
  }
  ...
  for (const n of flow.nodes) {
    ...
    const needsOutput =
      n.type.startsWith("input_") ||
      n.type.startsWith("bubble_") ||
      n.type.startsWith("contact_") ||
      n.type.startsWith("message_") ||
      n.type === "trigger";
    ...
    if (isBranching) {
      ...
      if (!hasTrue || !hasFalse)
        issues.push(`Conditional ${n.id} precisa das saídas true e false`);
    }
  }
  return issues;
}
...
export async function POST(
  ...
  if (parsed.data.enabled) {
    const issues = validateFlow(
      bot.flow_json as DsBotFlow,
      bot.start_node_id as string | null,
    );
    if (issues.length)
      return NextResponse.json(
        { error: "flow_invalid", issues },
        { status: 422 },
      );
  }
