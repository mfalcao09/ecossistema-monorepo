...
export const GET = protegerRota(
  async (req: NextRequest, _ctx) => {
    ...
    const counters = await Promise.all([
      supabase.from('deal_activities').select('id', { count: 'exact', head: true })
        .is('completed_at', null).gte('scheduled_at', nowIso),
      supabase.from('deal_activities').select('id', { count: 'exact', head: true })
        .is('completed_at', null).gte('scheduled_at', startOfDay()).lte('scheduled_at', endOfDay()),
      supabase.from('deal_activities').select('id', { count: 'exact', head: true })
        .is('completed_at', null).lt('scheduled_at', nowIso),
      supabase.from('deal_activities').select('id', { count: 'exact', head: true })
