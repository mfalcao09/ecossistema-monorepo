...
import { runDsVoiceTriggers } from "@/lib/atendimento/ds-voice-sender";
...
export async function GET(request: NextRequest) {
  ...
  console.log("[WEBHOOK] GET verificação recebida", {
    mode,
    token: token ? "***" : null,
  });
...
async function processarMensagemRecebida(
  ...
  try {
    ...
    if (!inboxResolvido) {
      console.error(
        "[WEBHOOK] Nenhum inbox WhatsApp encontrado para phone_number_id:",
        phoneNumberId,
      );
      return;
    }
    ...
    if (!inbox && inboxFallback) {
      console.warn(
        "[WEBHOOK] Usando inbox fallback — phone_number_id não bateu:",
        phoneNumberId,
      );
    }
    ...
    const conteudoTexto = msg.type === "text" ? msg.text.body : `[${msg.type}]`; // placeholder — Sprint S3: buscar mídia via Graph API
    ...
    if (errMsg) {
      ...
      // ── S9: avalia gatilhos DS Voice (fire-and-forget) ────────────────────
      // Enqueue funnel_executions — worker cron drena a cada 1min.
      if (isDsVoiceEnabled()) {
        runDsVoiceTriggers({
          type: "message_received",
          content: msg.type === "text" ? conteudoTexto : null,
          conversation_id: conversationId,
          contact_id: contactId,
          inbox_id: inboxId,
          channel: "whatsapp",
          ...
          console.error(
            "[WEBHOOK] runDsVoiceTriggers failed (não afeta ACK)",
            err,
          );
        });
      }

      // ── S9: transcrição de áudio via Gemini (fire-and-forget) ─────────────
      if (msg.type === "audio") {
        void (async () => {
          try {
            const enabled = await isIaTranscriptionEnabled();
            if (!enabled) return;
            // Busca o id persistido pelo wamid
            ...
            console.error(
              "[WEBHOOK] transcribeAudio failed (não afeta ACK)",
              err,
            );
          }
        })();
...
async function processarStatusMensagem(status: MetaStatusUpdate) {
  try {
    ...
    if (error) {
      console.warn(
        "[WEBHOOK] Status update — mensagem não encontrada ou erro",
        {
          wamid: status.id,
