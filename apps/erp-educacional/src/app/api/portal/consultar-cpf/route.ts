...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const ambientes = Array.from(
      new Set((rawDiplomas ?? []).map((d: any) => d.ambiente as string)),
    );
    ...
    for (const amb of ambientes) {
      const { data: cfg } = await admin
        ...
        .single();
      if (cfg) {
        iesNomePorAmbiente[amb] =
          (cfg as any).ies_nome ?? "Faculdades Integradas de Cassilândia";
      }
    }
    ...
    const diplomas: DiplomaSimplificado[] = (rawDiplomas ?? []).map(
      (d: any) => {
        const codigoVal =
          (d.codigo_validacao as string | null) ??
          `FIC-${(d.id as string).slice(0, 8).toUpperCase()}`;
        const urlVerif =
          (d.url_verificacao as string | null) ??
          `${baseUrl}/verificar/${codigoVal}`;
        const cursoNome =
          (d.curso?.nome as string | null) ??
          (d.titulo_conferido as string | null) ??
          "Diploma";
        const iesNome =
          iesNomePorAmbiente[d.ambiente as string] ??
