...
// ── GET: listar skills ──────────────────────────────────────────────────────
export const GET = protegerRota(async (_req: NextRequest) => {
...
export const POST = protegerRota(async (req: NextRequest, { userId }) => {
  ...
  let indexacao: { chunks_gerados: number; tokens_usados: number } | null = null
  try {
    indexacao = await indexarSkill(data.id, nome, conteudo, 1)
    console.log(`[RAG] Skill "${nome}" indexada: ${indexacao.chunks_gerados} chunks`)
  } catch (err) {
