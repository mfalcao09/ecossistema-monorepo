...
async function getExtractionAgentConfig(): Promise<AgentConfig> {
  ...
  if (provErr || !provider) {
    throw new Error(
      `Provider ${agente.provider_id} não encontrado ou inativo.`,
    );
  }
...
async function chamarGoogleAI(
  ...
  if (!res.ok) {
    ...
    console.error(
      `[google-ai] Erro HTTP ${res.status}: ${errorText.substring(0, 500)}`,
    );
    throw new Error(
      `Google AI retornou erro ${res.status}: ${errorText.substring(0, 200)}`,
    );
  }
  ...
  console.log(
    `[google-ai] finishReason=${finishReason}, text=${text.length} chars, usage=${JSON.stringify(usage)}`,
  );
  ...
  async (req: NextRequest, { userId, tenantId }) => {
    try {
      ...
      if (
        ...
        return NextResponse.json(
          { error: "Nenhum documento recebido" },
          { status: 400 },
        );
      }
      ...
      console.log(
        `[processar-docs] Recebidos ${documentos.length} documento(s)`,
      );

      // 3. Buscar configuração do agente de extração
      const config = await getExtractionAgentConfig();
      console.log(
        `[processar-docs] Agente: ${config.modelo} via ${config.provider.nome} (${config.provider.formato_api})`,
      );
      ...
      if (config.provider.formato_api === "google_genai") {
        ...
        for (const doc of documentos) {
          ...
          const isInlineData =
            doc.tipo === "imagem" &&
            (doc.conteudo.startsWith("data:image/") ||
              doc.conteudo.startsWith("data:application/pdf"));
          if (isInlineData) {
            const base64Match = doc.conteudo.match(
              /^data:([^;]+);base64,(.+)$/,
            );
            if (base64Match) {
              parts.push({
              ...
              parts.push({
                text: `[${isPdf ? "Documento PDF" : "Página"}: ${doc.nome}]`,
              });
              imageCount++;
              if (isPdf)
                console.log(
                  `[processar-docs] PDF nativo enviado como inlineData: ${doc.nome} (${Math.round(base64Match[2].length / 1024)}KB)`,
                );
            }
          } else {
            const texto =
              doc.conteudo.length > 15000
        ...
        console.log(
          `[processar-docs] Montado: ${imageCount} imagens + ${textCount} textos para Google AI REST`,
        );
        ...
        if (
          ...
          console.warn(
            `[processar-docs] ⚠️ ATENÇÃO: IA parou por ${result.finishReason}! JSON pode estar TRUNCADO. text=${result.text.length} chars`,
          );
        }
        ...
        try {
          ...
          const jsonStr = jsonMatch
            ? jsonMatch[1]
            : result.text.substring(result.text.indexOf("{"));
          const parsed = JSON.parse(
            jsonStr.substring(0, jsonStr.lastIndexOf("}") + 1),
          );
          const extraidos = parsed.dados_extraidos || parsed;
          const hasDocentes =
            Array.isArray(extraidos.docentes_horario) &&
            extraidos.docentes_horario.length > 0;
          const hasDisciplinas =
            Array.isArray(extraidos.disciplinas) &&
            extraidos.disciplinas.length > 0;
          console.log(
            `[processar-docs] DIAGNÓSTICO IA: disciplinas=${hasDisciplinas ? extraidos.disciplinas.length : 0}, docentes_horario=${hasDocentes ? extraidos.docentes_horario.length : 0}, keys=[${Object.keys(extraidos).join(",")}]`,
          );
          if (hasDocentes) {
            console.log(
              `[processar-docs] DOCENTES_HORARIO:`,
              JSON.stringify(extraidos.docentes_horario.slice(0, 3)),
            );
          }
        } catch (parseErr) {
          console.log(
            `[processar-docs] DIAGNÓSTICO: falha ao parsear JSON da IA (${(parseErr as Error).message})`,
          );
        }
        ...
        console.log(
          `[processar-docs] Montado: ${imageCount} imagens + ${textCount} textos para AI SDK`,
        );
        ...
        const client = createOpenAI({
          ...
          headers:
            config.provider.slug === "openrouter"
              ? {
        ...
        const elapsed = Date.now() - startTime;
        console.log(
          `[processar-docs] AI SDK: ${result.text.length} chars, finishReason=${result.finishReason}, ${elapsed}ms`,
        );
      ...
      return NextResponse.json(
        { error: `Erro ao processar documentos: ${msg}` },
        { status: 500 },
      );
    }
  },
