...
export async function POST(req: NextRequest) {
  ...
  if (error || !skills) {
    return NextResponse.json(
      { error: error?.message || "Erro ao buscar skills" },
      { status: 500 },
    );
  }
  ...
  if (!force) {
    ...
    skillsParaIndexar = skills.filter(
      (s: { id: string }) => !idsComChunks.has(s.id),
    );
  }
  ...
  for (const skill of skillsParaIndexar as Array<{
    id: string;
    nome: string;
    conteudo: string;
    versao: number;
    ...
    try {
      ...
      console.log(
        `[reindex] ✅ "${skill.nome}" → ${resultado.chunks_gerados} chunks`,
      );
    } catch (err) {
