...
async function convertPdfToPng(pdfBytes: Uint8Array): Promise<Uint8Array | null> {
  try {
    ...
    if (typeof OffscreenCanvas !== 'undefined') {
      ...
      console.log('[diploma-upload] PDF convertido para PNG:', Math.round(arrayBuffer.byteLength / 1024), 'KB')
      return new Uint8Array(arrayBuffer)
    }

    console.warn('[diploma-upload] OffscreenCanvas não disponível — PDF não convertido')
    return null
  } catch (err) {
...
export const POST = protegerRota(async (request, { userId }) => {
  ...
  if (tipo === 'timbrado-historico' && file.type === 'application/pdf') {
    console.log('[diploma-upload] Timbrado PDF detectado, tentando converter para PNG...')
    ...
    if (pngBuffer) {
      ...
      console.warn('[diploma-upload] Conversão PDF→PNG falhou, salvando PDF original')
    }
  }
