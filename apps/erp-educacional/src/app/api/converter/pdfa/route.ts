...
import { convertDocumentToPdfA, checkConverterHealth } from '@/lib/document-converter/client'
  ...
  try {
    ...
    if (!file) {
      return NextResponse.json(
        { error: 'Nenhum arquivo enviado. Use o campo "file" em multipart/form-data.' },
        { status: 400 }
      )
    }
      ...
      return NextResponse.json(
        {
          ...
          detail: 'O microserviço DocumentConverter não respondeu ao health check. Verifique se está rodando.'
        },
        { status: 503 }
      )
    }
    ...
    return NextResponse.json(
      { erro: sanitizarErro('Erro interno ao converter o documento.', 500), detail: message },
      { status: 500 }
    )
  }
})
