...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!docRow) {
    ...
    console.warn('[Webhook BRy PDF] documento não encontrado:', documentId, externalId)
  ...
  if (normalizado === 'CONCLUIDO' || normalizado === 'SIGNED' || normalizado === 'COMPLETED' || normalizado === 'FINISHED') {
  ...
  } else if (normalizado === 'CANCELADO' || normalizado === 'CANCELED' || normalizado === 'CANCELLED') {
    await supabase
      .from('diploma_documentos_complementares')
      .update({
        status: 'erro',
...
async function verificarEAvancarStatusDiploma(
  ...
  const todosAssinados = docs.every((d: { status: string }) => d.status === 'assinado')
  ...
  const statusQuePermitemAvancar = ['aguardando_documentos', 'documentos_assinados']
  if (statusQuePermitemAvancar.includes(diploma.status)) {
    ...
    console.log(`[Webhook BRy PDF] Diploma ${diplomaId} avançado para aguardando_envio_registradora`)
  }
}
