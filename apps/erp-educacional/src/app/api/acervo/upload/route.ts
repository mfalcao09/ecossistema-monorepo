...
export const POST = protegerRota(
  async (request: NextRequest, { userId, tenantId }) => {
    try {
      ...
      const localDigitalizacao =
        (formData.get("local_digitalizacao") as string) || "Secretaria FIC";
      const responsavelNome =
        (formData.get("responsavel_nome") as string) || "Secretaria";
      const responsavelCargo = formData.get("responsavel_cargo") as
        | string
        | null;
      const responsavelCpf = formData.get("responsavel_cpf") as string | null;
      const dataDocOriginal = formData.get("data_documento_original") as
        | string
        | null;
      const numeroDocOriginal = formData.get("numero_documento_original") as
        | string
        | null;
      const observacoes = formData.get("observacoes_originais") as
        | string
        | null;
      ...
      const resultado = await registrarDocumento({
        ...
        destinatario_cpf: formData.get("destinatario_cpf") as
          | string
          | undefined,
        titulo,
        numero_documento: formData.get("numero_documento") as
          | string
          | undefined,
