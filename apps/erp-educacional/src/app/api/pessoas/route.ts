...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filtros: PessoaFiltros = {
      ...
      tipo_vinculo:
        (searchParams.get("tipo_vinculo") as TipoVinculo) || undefined,
      ...
      ordenar_por:
        (searchParams.get("ordenar_por") as PessoaFiltros["ordenar_por"]) ||
        "nome",
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { categorias, ...pessoaInput } = body as PessoaCreateInput & {
      categorias?: TipoVinculo[];
    };
    ...
    if (categorias && categorias.length > 0) {
      ...
      if (vinculoError) {
        console.error(
          "[POST /api/pessoas] Erro ao criar vínculos:",
          vinculoError,
        );
