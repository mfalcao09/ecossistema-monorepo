...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      {
        sucesso: true,
        dados: override,
        mensagem: `Override "${body.tipo}" criado com sucesso.`,
      },
      { status: 201 },
    );
  } catch (erro: any) {
    ...
    if (erro.message?.includes("Já existe")) {
      return NextResponse.json(
        { sucesso: false, erro: "Registro duplicado." },
        { status: 409 },
      );
    }
    return erroInterno();
...
export async function DELETE(
  ...
  try {
    ...
    if (acao === "revogar") {
      ...
      return NextResponse.json({
        sucesso: true,
        mensagem: "Override revogado (desativado).",
      });
    } else {
      ...
      return NextResponse.json({
        sucesso: true,
        mensagem: "Override excluído definitivamente.",
      });
    }
  } catch (erro) {
