...
export function checkDiplomado(d: DadosDiplomadoAuditoria): IssueAuditoria[] {
  ...
  // (entra no SHA256 do código de validação do histórico, IN 05/2024).
  // Se vazio, a geração de XML falha mesmo com tudo o mais OK. A auditoria
  // precisa avisar antes da secretária clicar Gerar XMLs e ver erro.
  if (!d.ra?.trim()) {
    issues.push({
      campo: "ra",
      mensagem:
        "RA (registro acadêmico) do diplomando não preenchido. " +
        "Cadastrado pela secretaria — não vem do RG/Histórico do EM.",
      severidade: "critico",
      acao: "editar_diplomado",
      valor_atual: d.ra,
    });
  ...
  if (!temCodIBGE) {
    issues.push({
      ...
      mensagem:
        "Código IBGE do município de naturalidade não preenchido (XSD exige 7 dígitos).",
      severidade: "critico",
      acao: "editar_diplomado",
  ...
  if (!d.rg_numero?.trim()) {
    issues.push({
      campo: "rg_numero",
      mensagem:
        "Número do RG não preenchido. O XSD exige RG ou Outro Documento de Identificação.",
      severidade: "critico",
      acao: "editar_diplomado",
