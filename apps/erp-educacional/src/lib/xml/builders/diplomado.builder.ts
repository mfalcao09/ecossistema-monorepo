...
export function buildDiplomado(
  ...
  if (d.naturalidade_municipio_estrangeiro) {
    naturalidade
      .ele("NomeMunicipioEstrangeiro")
      .txt(d.naturalidade_municipio_estrangeiro);
  } else {
  ...
  if (d.rg_numero) {
    ...
    // XSD TNumeroRG pattern [a-zA-Z0-9]{4,15} — remove pontuação (pontos, traços, espaços)
    // antes de gravar. Ex: "123.456.789-09" → "12345678909". O valor original
    // permanece no banco; a sanitização é apenas para o XML.
    const rgNumeroXSD = d.rg_numero.replace(/[^a-zA-Z0-9]/g, "").slice(0, 15);
    rg.ele("Numero").txt(rgNumeroXSD || d.rg_numero.slice(0, 15));
