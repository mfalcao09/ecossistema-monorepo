...
function buildDocentes(parent: XMLBuilder, docentes: DocenteInfo[], wrapperTag?: string): void {
...
function buildCargaHorariaEtiqueta(parent: XMLBuilder, chs: CargaHorariaComEtiqueta[]): void {
  for (const ch of chs) {
...
function buildCargaHorariaRelogioEtiqueta(parent: XMLBuilder, chs: CargaHorariaRelogioComEtiqueta[]): void {
  for (const ch of chs) {
...
function buildDisciplina(parent: XMLBuilder, disc: Disciplina): void {
  ...
  if (disc.situacao === 'Aprovado') {
    ...
    if (disc.forma_integralizacao) {
      ...
      aprovado.ele('OutraFormaIntegralizacao').txt(disc.outra_forma_integralizacao);
    }
...
function buildAtividadeComplementar(parent: XMLBuilder, ac: AtividadeComplementar): void {
  ...
  eleOpc(atividade, 'DataRegistro', ac.data_registro ? fmtData(ac.data_registro) : undefined);
  ...
  buildDocentes(atividade, ac.docentes_validacao, 'DocentesResponsaveisPelaValidacao');
}
...
function buildEstagio(parent: XMLBuilder, est: Estagio): void {
  ...
  estagio.ele('CodigoUnidadeCurricular').txt(est.codigo_unidade_curricular || '');
 ...
 */
export function buildHistoricoEscolar(parent: XMLBuilder, dados: DadosDiploma): void {
