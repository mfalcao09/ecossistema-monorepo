...
export function aplicarSnapshotSobreDadosDiploma(
  ...
  if (s_dip) {
    ...
    if (s_dip.data_nascimento)
      out.diplomado.data_nascimento = s_dip.data_nascimento;
    if (s_dip.sexo) {
    ...
    if (s_dip.naturalidade_uf)
      out.diplomado.naturalidade_uf = s_dip.naturalidade_uf;
  ...
  if (s_cur) {
    ...
    if (s_cur.numero_reconhecimento) {
      const ato: AtoRegulatorio = {
        ...
        data_publicacao:
          s_cur.dou_reconhecimento || out.curso.reconhecimento?.data_publicacao,
      };
    ...
    if (s_cur.numero_renovacao) {
      const ato: AtoRegulatorio = {
        ...
        data:
          s_cur.data_renovacao ||
          out.curso.renovacao_reconhecimento?.data ||
          "",
      };
      if (out.curso.renovacao_reconhecimento) {
        ato.veiculo_publicacao =
          out.curso.renovacao_reconhecimento.veiculo_publicacao;
        ato.data_publicacao =
          out.curso.renovacao_reconhecimento.data_publicacao;
        ato.secao_publicacao =
          out.curso.renovacao_reconhecimento.secao_publicacao;
        ato.pagina_publicacao =
          out.curso.renovacao_reconhecimento.pagina_publicacao;
  ...
  if (s_da) {
    if (s_da.data_colacao_grau)
      out.diploma.data_colacao_grau = s_da.data_colacao_grau;
  ...
  if (s_da) {
    ...
    if (s_da.carga_horaria_integralizada != null) {
      out.historico.carga_horaria_integralizada =
        s_da.carga_horaria_integralizada;
    }
    // situacao_discente — data_conclusao/colacao já vêm do snapshot
    out.historico.situacao_discente = {
      tipo: "Formado",
      data_conclusao:
        s_da.data_conclusao || out.historico.situacao_discente.data_conclusao,
      data_colacao_grau:
        s_da.data_colacao_grau ||
        out.historico.situacao_discente.data_colacao_grau,
    };
  }

  // Disciplinas — sobrescreve inteiro se snapshot tem lista
  if (Array.isArray(snapshot.disciplinas) && snapshot.disciplinas.length > 0) {
    out.historico.disciplinas = snapshot.disciplinas.map(
      snapshotDisciplinaParaDadosDiploma,
    );
  }

  // Atividades complementares — sobrescreve inteiro
  if (Array.isArray(snapshot.atividades_complementares)) {
    const ativs: AtividadeComplementar[] =
      snapshot.atividades_complementares.map(
        (a, idx): AtividadeComplementar => ({
          ...
          carga_horaria_relogio: [
            {
              valor: a.carga_horaria_relogio ?? 0,
            } as CargaHorariaRelogioComEtiqueta,
          ],
          docentes_validacao: [],
        }),
      );
    out.historico.atividades_complementares =
      ativs.length > 0 ? ativs : undefined;
  }

  // Estágios — sobrescreve inteiro
  if (Array.isArray(snapshot.estagios)) {
    const esgs: Estagio[] = snapshot.estagios.map(
      (e, idx): Estagio => ({
  ...
  if (Array.isArray(snapshot.assinantes) && snapshot.assinantes.length > 0) {
    ...
    out.assinantes = doSnap.map((snap, idx): Assinante => {
      ...
      return {
        ...
        tipo_certificado:
          original?.tipo_certificado ?? (snap.cpf ? "eCPF" : "eCNPJ"),
...
function snapshotDisciplinaParaDadosDiploma(d: SnapshotDisciplina): Disciplina {
  ...
  else if (s.includes("curs") || s.includes("pendente") || s.includes("tranc"))
    situacao = "Pendente";

  // Forma integralização — só aceita valores do XSD v1.05
  // (Cursado | Validado | Aproveitado)
  const formaIntegralizacaoMap: Record<
    string,
    "Cursado" | "Validado" | "Aproveitado"
  > = {
    cursado: "Cursado",
...
function normalizarTitulacao(
  ...
  if (s.includes("pos-doc") || s.includes("pós-doc") || s.includes("pós doc"))
    return "Doutorado";
  if (s.includes("mestr")) return "Mestrado";
  if (
    s.includes("especiali") ||
    s.includes("especialização") ||
    s.includes("pós") ||
    s.includes("pos-grad") ||
    s.includes("pós-grad") ||
    s.includes("mba")
  ...
  if (s.includes("gradu") || s.includes("bachar") || s.includes("licen"))
    return "Graduação";
  return "Graduação";
}
...
function mapearFormaAcesso(raw: string | null | undefined): TFormaAcesso {
  ...
  if (s.includes("bi") || s.includes("li") || s.includes("egresso"))
    return "Egresso BI/LI";
  if (s.includes("pec-g") || s.includes("pec g") || s.includes("pecg"))
    return "PEC-G";
  if (s.includes("ex officio") || s.includes("transferência"))
    return "Transferência Ex Officio";
  if (s.includes("judicial")) return "Decisão judicial";
  if (s.includes("remanescente")) return "Seleção para Vagas Remanescentes";
  if (s.includes("programa especial"))
    return "Seleção para Vagas de Programas Especiais";
  return "Vestibular";
}
