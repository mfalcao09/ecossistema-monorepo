...
// ──────────────────────────────────────────────────────────────
export type TriggerNode = BaseNode<"trigger", { label?: string }>;
...
export type InputTextNode = BaseNode<
  "input_text",
  InputBaseData & { min_length?: number; max_length?: number }
>;
export type InputNumberNode = BaseNode<
  "input_number",
  InputBaseData & { min?: number; max?: number }
>;
...
export type InputDateNode = BaseNode<
  "input_date",
  InputBaseData & { min_date?: string; max_date?: string }
>;
export type InputPhoneNode = BaseNode<
  "input_phone",
  InputBaseData & { country?: string }
>;
export type InputButtonNode = BaseNode<
  "input_button",
  InputBaseData & {
    options: Array<{ id: string; label: string; value: string }>;
  }
>;
export type InputFileNode = BaseNode<
  "input_file",
  InputBaseData & { accept?: string; max_mb?: number; bucket?: string }
>;
...
export type ContactRemoveTagNode = BaseNode<
  "contact_remove_tag",
  { tag: string }
>;
export type ContactUpdateFieldNode = BaseNode<
  "contact_update_field",
  { field: string; value: string; interpolate?: boolean }
>;
...
export type DsBotNode =
  ...
  | FlowWaitNode
  | ContactAddTagNode
  | ContactRemoveTagNode
  | ContactUpdateFieldNode
  | MessageWabaTemplateNode
  | MessageDsVoiceNode
  | MessageForwardNode
  | AttendanceTransferQueueNode
  | AttendanceAssignAgentNode
  | AttendanceOpenProtocolNode
  | AttendanceCloseNode
  | ConditionalNode
  | AgentHandoffNode;
...
export const NODE_CATEGORIES: Record<
  NodeCategory,
  { label: string; types: DsBotNodeType[] }
> = {
  ...
  flow: {
    label: "Fluxo",
    types: ["flow_goto", "flow_back", "flow_end", "flow_wait"],
  },
  contact: {
    label: "Contato",
    types: ["contact_add_tag", "contact_remove_tag", "contact_update_field"],
  },
  message: {
    label: "Mensagem",
    types: ["message_waba_template", "message_ds_voice", "message_forward"],
  },
  attendance: {
    label: "Atendimento",
    types: [
...
export function nodeCategory(type: DsBotNodeType): NodeCategory {
  for (const [cat, meta] of Object.entries(NODE_CATEGORIES) as [
    NodeCategory,
    { types: DsBotNodeType[] },
  ][]) {
