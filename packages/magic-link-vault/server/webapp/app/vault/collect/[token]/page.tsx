...
import { encryptClientSide, importDEK, base64ToArrayBuffer } from '@ecossistema/magic-link-vault';
      ...
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md text-center p-8">
          ...
          <p className="text-gray-500 text-sm">
            Este link é de uso único e expira em 15 minutos. Solicite um novo link ao agente.
          </p>
        </div>
      </main>
    );
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md text-center p-8">
          ...
          <h1 className="text-xl font-semibold text-gray-800 mb-2">Credencial armazenada com segurança</h1>
          <p className="text-gray-500 text-sm">
            O valor foi cifrado no seu navegador com AES-256-GCM antes de ser enviado.
            O servidor armazenou apenas o texto cifrado. Este link foi invalidado.
          </p>
        </div>
      </main>
    );
      ...
      <div className="max-w-md w-full bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div className="mb-6">
          ...
          <p className="text-xs text-orange-500 mt-2">
            ⏱ Expira em aproximadamente {metadata?.expires_in_minutes ?? '—'} minutos
          </p>
        </div>
        ...
        {status === 'error' && errorMsg && (
          <p className="text-sm text-red-600 mt-3 bg-red-50 rounded p-2">{errorMsg}</p>
        )}
        ...
        >
          {status === 'submitting' ? 'Cifrando e enviando…' : 'Enviar com segurança'}
        </button>
      </div>
    </main>
  );
