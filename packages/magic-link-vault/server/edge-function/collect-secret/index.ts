...
Deno.serve(async (req: Request) => {
  ...
  let body: { token: string; encrypted_payload: { ciphertext: string; iv: string; algorithm: string; version: string } };
  try {
    body = await req.json();
  } catch {
  ...
  if (!token || !encrypted_payload?.ciphertext || !encrypted_payload?.iv) {
    return err(400, 'missing_fields', 'token and encrypted_payload (ciphertext, iv) are required');
  }
  ...
  if (tokenErr || !tokenRow) {
    return err(400, 'invalid_or_expired_token', 'Token inválido, já utilizado ou expirado');
  }
