...
Deno.serve(async (req: Request) => {
  ...
  const allowed = acl.some(
    (entry) =>
      new RegExp(`^${entry.agent_pattern.replace("*", ".*")}$`).test(
        agent_id,
  ...
  if (!allowed) {
    return err(
      403,
      "acl_denied",
      `Agent ${agent_id} não tem permissão 'proxy' para ${credential_name}`,
    );
  }
  ...
  if (!kekHex)
    return err(500, "kek_missing", "KEK não configurada no ambiente");
  ...
  try {
    ...
    return err(
      500,
      "decrypt_failed",
      "Falha na decifração AES-GCM (auth tag mismatch?)",
    );
  }
