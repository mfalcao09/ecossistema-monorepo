...
describe("AES-256-GCM round-trip", () => {
  ...
  test("ciphertext adulterado falha com auth tag mismatch", async () => {
    ...
    await expect(decryptServerSide(tampered, dekRaw)).rejects.toThrow(
      /auth tag mismatch|tampered/i,
    );
  });
  ...
  test("DEK errada falha na decifração", async () => {
    ...
    await expect(decryptServerSide(encrypted, wrongDEK)).rejects.toThrow(
      /auth tag|Decryption failed/i,
    );
  });
});
