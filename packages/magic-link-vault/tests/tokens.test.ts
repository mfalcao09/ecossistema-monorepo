...
describe('buildNewToken', () => {
  ...
  test('TTL padrão é 15 minutos', () => {
    ...
    const result = buildNewToken({
      credential_name: 'X', project: 'ecosystem', scope_description: 'X', requested_by: 'agent',
    });
  ...
  test('TTL acima de 60 lança TokenError', () => {
    expect(() =>
      buildNewToken({ credential_name: 'X', project: 'ecosystem', scope_description: 'X', requested_by: 'agent', ttl_minutes: 61 }),
    ).toThrow(TokenError);
  });

  test('TTL abaixo de 1 lança TokenError', () => {
    expect(() =>
      buildNewToken({ credential_name: 'X', project: 'ecosystem', scope_description: 'X', requested_by: 'agent', ttl_minutes: 0 }),
    ).toThrow(TokenError);
  });
});
...
describe('assertTokenValid', () => {
  ...
  test('token already used lança TokenError', () => {
    expect(() => assertTokenValid(makeToken({ used: true }), 'abc')).toThrow(/already used/i);
  });

  test('token expirado lança TokenError', () => {
    const past = new Date(Date.now() - 60_000).toISOString();
    expect(() => assertTokenValid(makeToken({ expires_at: past }), 'abc')).toThrow(/expired/i);
  });
});
