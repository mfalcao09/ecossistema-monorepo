...
| P-040 | ATND-S5 | config | high | Criar 1º template WABA na Meta Business Manager (ex: `fic_boas_vindas_matricula` UTILITY) e aguardar aprovação 24-48h — destrava envio ativo e modal "janela fechada" | envio ativo WABA em prod | 2026-04-20 |
| P-041 | ATND-Setup | test | low | Abrir `/dev/tokens` em staging para QA visual dos design tokens Nexvy (antes de S4/S5/S6 começarem a usar) — validar paridade visual | paridade visual com Nexvy | 2026-04-20 |
| P-042 | ATND-S4 | seed | med | Exportar 245 contatos + 171 deals reais do Nexvy (console.nexvy.tech → API → export) para alimentar script `nexvy_import.ts` quando S4 estiver em staging | migração 1:1 do Nexvy → ERP | 2026-04-20 |
