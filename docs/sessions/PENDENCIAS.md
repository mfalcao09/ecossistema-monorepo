...
| P-009 | S16 | config | high | Provisionar valores reais das 5 credenciais sandbox CFO-FIC no ECOSYSTEM (`is_active=false` → `true`). **Inter:** acessar developers.inter.co → Sandbox → criar app → copiar client_id, client_secret, baixar cert .crt e .key (converter para PEM). **Evolution:** painel Evolution API → instance `fic-sandbox` → API Key. Atualizar via `UPDATE ecosystem_credentials SET vault_key='<valor>', is_active=true WHERE project='fic' AND environment='sandbox' AND name='<NOME>';` | primeira execução real do CFO-FIC (régua de cobrança, boletos) | 2026-04-17 |
